...
const { emoji } = require("../../sources/2048");
...
class Game {
  ...
  moveLeft() {
    const cache = JSON.stringify(this.board.data);
      ...
      for (let x = 0; x < this.board.data[y].length; x++) {
        ...
        while (copyX < this.board.data[y].length) {
          if (this.board.data[y][copyX] != 0 && this.board.data[y][x] == 0) {
            ...
            this.board.data[y][copyX] == this.board.data[y][x] &&
            this.board.data[y][x] != 0
          ) {
      ...
      for (let y = 0; y < this.board.data.length; y++) {
        ...
        while (copyY < this.board.data[y].length) {
          if (this.board.data[copyY][x] != 0 && this.board.data[y][x] == 0) {
            ...
            this.board.data[copyY][x] == this.board.data[y][x] &&
            this.board.data[y][x] != 0
          ) {
  ...
  _checkWin() {
    this.board.topTile = Math.max.apply(
      null,
      this.board.data.map((e) => Math.max.apply(null, e))
    );
