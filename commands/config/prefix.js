...
module.exports = {
  ...
  description:
    "Change the prefix of the server to a new one or check to see the curent prefix",
    ...
    if (args[0]) {
      if (args[0].split("").length > 5) {
        return message.channel.send({
          embeds: [
            new MessageEmbed()
              .setAuthor({
                name: "The prefix max length is 5 characters",
      ...
      message.channel.send({
        embeds: [
          new MessageEmbed()
            .setAuthor({
              name: `Successfully updated server prefix to ${args[0]}`,
              iconURL: message.author.displayAvatarURL(),
            })
      ...
      message.channel.send({
        embeds: [
          new MessageEmbed()
            .setAuthor({
              name: `Current server prefix is ${prefix}`,
              iconURL: message.author.displayAvatarURL(),
            })
