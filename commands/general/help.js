...
module.exports = {
    ...
    run: function(message, args, prefix) {
        ...
        if (client.commands.has(args[0].toLowerCase())) {
            ...
            let embed = new MessageEmbed().setAuthor({name: `Help menu`, iconURL: message.author.displayAvatarURL()}).setColor(colors.info).setDescription(`Use ${prefix}help [command] to see more info on all the commands`).setFooter(`Requested:`, client.user.displayAvatarURL()).setTimestamp(Date.now());
